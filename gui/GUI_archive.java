...
public class GUI_archive extends Application {
	...
	final static int TABPANE_WIDTH = 3* WINDOW_WIDTH/4;
	final static int TABPANE_HEIGHT = 3*WINDOW_HEIGHT/4;
	...
	public static gui.Controller controller = new Controller(); 
	...
	public void start(Stage primaryStage) {
		...
		sp = new ScrollPane(logVBox);
		sp.setFitToHeight(true);
		sp.setVbarPolicy(ScrollBarPolicy.ALWAYS);
		sp.setHbarPolicy(ScrollBarPolicy.NEVER);
		tabPane.getTabs().get(LOG).setContent(sp);//logVBox);
		...
		Scene scene = new Scene(border, WINDOW_WIDTH, WINDOW_HEIGHT);//border.getPrefWidth(), border.getPrefHeight());
		userTextField.requestFocus();
		primaryStage.setScene(scene);
	    primaryStage.setTitle(APP_TITLE);
	    scene.getStylesheets().add(GUI_archive.class.getResource(FILE_CSS).toExternalForm());
	    primaryStage.show();
	}
