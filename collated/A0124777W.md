...
public class DeadlineTask extends Task {
	...
	public DeadlineTask(String name, String details, int id,
			boolean isCompleted, ArrayList<String> tags, TaskType taskType,
			Calendar end) {
	...
	public DeadlineTask(Task task, Calendar end) {
		super(task.getName(), task.getDescription(), task.getId(), task
				.getIsCompleted(), task.getTags(), TaskType.DEADLINE_TASK);
		this.end = end;
	}

	public DeadlineTask(DeadlineTask newTask) {
		super(newTask.getName(), newTask.getDescription(), newTask.getId(),
				newTask.getIsCompleted(), newTask.getTags(), newTask
						.getTaskType());
...
public class Event extends Task {
	...
	public Event(Task task, Calendar start, Calendar end) {
		super(task.getName(), task.getDescription(), task.getId(), task
				.getIsCompleted(), task.getTags(), TaskType.EVENT);
		this.start = start;
		this.end = end;
	}

	public Event(DeadlineTask dlTask, Calendar start) {
		super(dlTask.getName(), dlTask.getDescription(), dlTask.getId(), dlTask
				.getIsCompleted(), dlTask.getTags(), TaskType.EVENT);
		this.start = start;
		this.end = dlTask.getEnd();
	}

	public Event(String name, String details, int id, boolean isCompleted,
			ArrayList<String> tags, TaskType taskType, Calendar start,
			Calendar end) {
		super(name, details, id, isCompleted, tags, taskType);
		setStart(start);
		setEnd(end);
...
public class Logic {
	...
	// Date formatter for
	public static final SimpleDateFormat displayDateFormatter = new SimpleDateFormat(
			"dd-MM-yyyy, HH:mm");
	...
	private Model executeSearch(ParsedCommand parsedCommand) {
		...
		try {
			...
			tasksToDisplay = search.multiSearch(storage.getAllTasks(),
					parsedCommand);
	...
	/*
	 * Issues the next available ID. IDs take integer values and are not
	 * replaced when deleted.
	 */
	...
	/*
	 * Searches for all tasks from now to the end of the day. Returns the
	 * results in a List. Results are sorted by date with the earliest tasks
	 * appearing first while the later tasks appearing later.
	 * 
	 * @returns List<Task>
	 */
	public static List<Task> updateTodayList() {
		try {
			...
			List<Task> todayList = Search.searchDate(storage.getAllTasks(),
					fromCal, toCal);
	...
	/*
	 * Searches for all tasks that are due from today onwards. Returns the
	 * results in a List. Returns the results in a List. Results are sorted by
	 * date with the earliest tasks appearing first while the later tasks
	 * appearing later.
	 */
	public static List<Task> updateMainList() {
		try {
			...
			List<Task> mainList = Search.searchDate(storage.getAllTasks(),
					fromCal, toCal);
	...
	/*
	 * Searches for all Overdue tasks. Results are returned in a List. Results
	 * are sorted by date with the earliest tasks appearing first while the
	 * later tasks appearing later.
	 */
	public static List<Task> updateOverdueList() {
		try {
			...
			List<Task> incompleteOverdue = Search.searchDate(
					storage.getAllTasks(), fromCal, toCal);
			List<Task> overdue = Search.search(incompleteOverdue,
					INCOMPLTETED_TASKS);
...
public class Model {
	...
	public List<Task> getFloatingList() {
		return floatingList;
	}
	...
	public void updateSearch(String consoleMessage, ParsedCommand searchQuery,
	...
	public void updateSearchList() {
		try {
			Search search = new Search();
			this.searchList = search.multiSearch(storage.getAllTasks(),
					searchQuery);
		} catch (ParseException e) {
...
```
###### test\LogicTests\LogicTest.java
``` java
...
```
###### test\LogicTests\UpdateTest.java
``` java
public class UpdateTest {
	...
	public void testUpdateTask() throws Exception {
		...
		Task updated = command.updateTask(title, description, tags,
				isCompleted, firstDate, secondDate, toUpdate);
		assertEquals(true, taskChecker(updated, toUpdate));

		// test updating name
		title = "short";
		correct.setName(title);
		updated = command.updateTask(title, description, tags, isCompleted,
				firstDate, secondDate, toUpdate);
	...
	public boolean taskChecker(Task task1, Task task2) {
		if (task1.getId() != task2.getId()) {
			...
			if (task1.getTaskType() == null && task2.getTaskType() == null) {
				// do nothing
			} else if (task1.getTaskType() != null
					&& task2.getTaskType() != null) {
